...
router.get("/org-chart/project", (req, res) => {
  ...
  projects.forEach((project) => {
    ...
    // Find the lead for this project
    ...
    if (lead) {
      ...
      result.push({
        id: leadId,
        info: lead,
        parentId: projectId, // Lead under the project
        level: 2,
      });
    }

    // Find team members who are working on this project
    employees
      .filter((emp) => emp.project === project.name) // Match project name
      .forEach((teamMember) => {
        result.push({
          id: cid++,
          info: teamMember,
          parentId: leadId || projectId, // If no lead, assign directly to project
          level: 3,
        });
      });
  });
