...
export class ProjectListTableComponent implements OnInit, OnChanges {
  ...
  delete(name: string) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: 'Deleted!',
          text: 'Project data has been deleted.',
          icon: 'success',
        });
      }
    });
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.searchObj['global_term' as keyof ProjectSearchModel] = this.term;
    this.filterProjects(this.searchObj);
    this.changeDetect();
  }

  filterProjects(searchObj: ProjectSearchModel) {
    const fieldFiltered = this.project.filter((pro) => {
      const fieldSearch = Object.entries(searchObj).every(([key, value]) => {
        if (key === 'global_term' || value === '') return true;
        return (pro as any)[key]
          ?.toString()
          .toLowerCase()
    ...
    this.filteredProjects = fieldFiltered.filter(
      (pro) =>
        pro.lead.toString().includes(lower) ||
        pro.name.toLowerCase().includes(lower) ||
        pro.manager.toLowerCase().includes(lower) 
    );
    this.searchCount.emit(this.filteredProjects.length);
    this.dataSource.data = this.filteredProjects;
    this.changeDetect();
  }
  ...
  generatePdf(){
    ...
    this.projectService.generatePDF(sortedFilteredData).subscribe(response => {
      const blob = new Blob([response], { type: 'application/pdf' });
      saveAs(blob, 'project_details.pdf');
    }, error => {
  ...
  view(name: string): void {
    this.router.navigate(['/employee-project-view'], { queryParams: { name: name } });
  }
}
